...
link:dbunithelper.zip[Here] is the code for the DBUnitHelper class.  Note that this is still experimental, but I've used it on three projects now with good success, but it does abuse the JPA API by talking to the TopLink objects directly, so it is definitely not portable.  You have been warned. :)
